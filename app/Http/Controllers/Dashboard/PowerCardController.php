...
class PowerCardController extends Controller
{
    public function index(Request $request, League $league): JsonResponse
    {
        ...
        return response()->json(['cards' => $league->powerCards()->where('season_id', $season->id)->where('user_id', $request->user()->id)->get()]);
    }
    ...
    public function store(Request $request, League $league): JsonResponse
    {
        ...
        if (! $season->selections()->where('user_id', $request->user()->id)->exists()) throw ValidationException::withMessages(['card' => 'Lock your squad before using a power card.']);
        if ($season->readyEntries()->where('user_id', $request->user()->id)->exists()) throw ValidationException::withMessages(['card' => 'Power cards are locked after you are ready.']);
        ...
        if ($league->powerCards()->where('season_id', $season->id)->where('user_id', $request->user()->id)->where('card_type', $validated['card_type'])->exists()) {
            throw ValidationException::withMessages(['card' => 'This power card has already been submitted.']);
        }

        $this->validateTarget($request, $league, $season->id, $validated);
        $card = $league->powerCards()->create([...$validated, 'season_id' => $season->id, 'user_id' => $request->user()->id]);
        return response()->json(['message' => ucfirst($card->card_type).' card submitted.', 'card' => $card], 201);
    }

    private function validateTarget(Request $request, League $league, int $seasonId, array $data): void
    {
        $ownSelections = \App\Models\LeagueSeasonSelection::where('season_id', $seasonId)->where('user_id', $request->user()->id)->pluck('player_id');
        ...
        if (! \App\Models\LeagueSeasonSelection::where('season_id', $seasonId)->where('user_id', $data['target_user_id'])->exists()) throw ValidationException::withMessages(['target_user_id' => 'That opponent has not locked a squad yet.']);
        ...
        $targetSelections = \App\Models\LeagueSeasonSelection::where('season_id', $seasonId)->where('user_id', $data['target_user_id'])->pluck('player_id');
