...
(() => {
  ...
  const render = () => { slots.innerHTML = ''; const byRole = {}; slotList().forEach(slot => (byRole[slotRole(slot)] ||= []).push(slot)); ['forward','midfielder','defender','goalkeeper'].forEach(role => { if (!byRole[role]) return; const row = document.createElement('div'); row.className = 'flex flex-wrap justify-center gap-3'; row.innerHTML = byRole[role].map(slot => `<button type="button" data-slot="${slot}" class="slot-button w-28 rounded-2xl border border-white/25 bg-black/20 p-3 text-center text-xs font-bold transition hover:border-uno-lime">${selected[slot] ? `<strong class="block truncate text-uno-lime">${escape(selected[slot].name)}</strong><small class="mt-1 block truncate text-white/45">${escape(selected[slot].team_name || selected[slot].position || '')}</small>` : `<span class="block text-white/55">+ ${labels[role]}</span><small class="mt-1 block text-white/35">Slot ${slot.split('_')[1] || ''}</small>`}</button>`).join(''); slots.appendChild(row); }); document.querySelector('[data-slot="coach"]').innerHTML = selected.coach ? `<strong class="block truncate text-uno-lime">${escape(selected.coach.name)}</strong><small class="mt-1 block truncate text-white/45">${escape(selected.coach.team_name || '')}</small>` : '+ Select coach'; const count = Object.keys(selected).length; if (progress) progress.textContent = `${count} / 12 selected`; const save = document.getElementById('saveSquad'); if (save) { save.disabled = count !== 12; save.classList.toggle('opacity-40', count !== 12); } };
  ...
  const showResults = (items) => { const selectedIds = new Set(Object.values(selected).map(p => Number(p.id))); items = items.filter(p => !selectedIds.has(Number(p.id))); results.innerHTML = items.length ? items.map(p => `<button type="button" data-player='${JSON.stringify(p).replace(/'/g, '&#039;')}' class="flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 p-3 text-left hover:border-uno-lime"><span class="grid h-10 w-10 shrink-0 place-items-center overflow-hidden rounded-full bg-uno-blue/30 text-uno-lime">${p.image_url ? `<img src="${escape(p.image_url)}" alt="" class="h-full w-full object-cover">` : '<i class="bx bx-user text-xl"></i>'}</span><span class="min-w-0"><strong class="block truncate text-sm">${escape(p.name)}</strong><small class="block truncate text-xs text-white/45">${escape(p.position || 'Football data')} · ${escape(p.team_name || 'Unknown team')}</small></span></button>`).join('') : '<p class="py-5 text-center text-sm text-white/45">No available players found.</p>'; };
