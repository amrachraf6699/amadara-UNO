...
@php
  ...
  ]])->toArray() : $draftSelections->mapWithKeys(fn ($selection) => [$selection->slot_key => [
...
<main data-dashboard-page="{{ $editable ? 'squad-builder' : 'member-squad' }}" class="hud-squad {{ $editable && $locked && $league->status === \App\Models\League::STATUS_YET_TO_START ? 'pb-56' : '' }} mx-auto min-h-[calc(100vh-150px)] max-w-6xl px-4 py-8 lg:px-8 lg:py-14">
  ...
  <section class="mt-8 glass-panel rounded-3xl p-5 sm:p-7">
    ...
    <select id="formation" class="mt-2 w-full max-w-xs rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white outline-none focus:border-uno-lime">
      @foreach ($formations as $formation => $counts)<option value="{{ $formation }}" @selected(($draft?->formation ?? array_key_first($formations)) === $formation)>{{ $formation }}</option>@endforeach
    </select>
  </section>
  @endif
...
(() => {
  ...
  const slots = document.getElementById('slots'), formation = document.getElementById('formation'), modal = document.getElementById('playerModal'), search = document.getElementById('playerSearch'), results = document.getElementById('searchResults'), more = document.getElementById('showMore');
  const selected = {...saved}, conflicts = new Set(); let activeSlot = null, timer = null, controller = null, page = 1;
  ...
  const render = () => { slots.innerHTML = ''; const byRole = {}; slotList().forEach(slot => (byRole[slotRole(slot)] ||= []).push(slot)); ['forward','midfielder','defender','goalkeeper'].forEach(role => { if (!byRole[role]) return; const row = document.createElement('div'); row.className = 'flex flex-wrap justify-center gap-3'; row.innerHTML = byRole[role].map(slot => { const player = selected[slot]; return `<div class="relative w-32">${player ? `<button type="button" data-remove="${slot}" class="absolute right-2 top-2 z-10 grid h-6 w-6 place-items-center rounded-lg border border-white/15 bg-[#071d33]/90 text-sm text-white/55 transition hover:border-red-300 hover:bg-red-400/15 hover:text-red-200" aria-label="Remove ${escape(player.known_name || player.name)}" title="Remove player"><i class="bx bx-trash"></i></button>` : ''}<button type="button" data-slot="${slot}" class="slot-button w-full rounded-2xl border border-white/25 bg-black/20 p-3 text-center text-xs font-bold transition hover:border-uno-lime ${player ? 'pt-8' : ''}">${player ? `<strong class="block truncate text-uno-lime">${escape(player.known_name || player.name)}</strong><small class="mt-1 block truncate text-white/60">${escape(player.team_name || player.nationality || '')}</small><small class="mt-1 block truncate text-white/40">${escape([player.nationality, player.age ? `${player.age} yrs` : '', player.height_cm ? `${player.height_cm} cm` : ''].filter(Boolean).join(' · '))}</small>` : `<span class="block text-white/55">+ ${labels[role]}</span><small class="mt-1 block text-white/35">Slot ${slot.split('_')[1] || ''}</small>`}</button></div>`; }).join(''); slots.appendChild(row); }); document.querySelector('[data-slot="coach"]').innerHTML = selected.coach ? `<strong class="block truncate text-uno-lime">${escape(selected.coach.known_name || selected.coach.name)}</strong><small class="mt-1 block truncate text-white/60">${escape(selected.coach.position || selected.coach.team_name || 'Coach')}</small><small class="mt-1 block truncate text-white/40">${escape([selected.coach.nationality, selected.coach.age ? `${selected.coach.age} yrs` : '', selected.coach.height_cm ? `${selected.coach.height_cm} cm` : ''].filter(Boolean).join(' · '))}</small>` : '+ Select coach'; const count = Object.keys(selected).length; const save = document.getElementById('saveSquad'); if (save) { save.disabled = count !== 12; save.classList.toggle('opacity-40', count !== 12); } };
  ...
  const decorate = () => { document.querySelectorAll('.slot-button').forEach(button => { const player = selected[button.dataset.slot]; const conflict = player && conflicts.has(Number(player.id)); button.classList.toggle('border-red-400', Boolean(conflict)); button.classList.toggle('ring-2', Boolean(conflict)); button.classList.toggle('ring-red-400', Boolean(conflict)); }); const coach = document.querySelector('[data-slot="coach"]'); const coachConflict = selected.coach && conflicts.has(Number(selected.coach.id)); coach?.classList.toggle('border-red-400', Boolean(coachConflict)); coach?.classList.toggle('ring-2', Boolean(coachConflict)); coach?.classList.toggle('ring-red-400', Boolean(coachConflict)); if (!selected.coach) coach?.parentElement.querySelector('[data-remove="coach"]')?.remove(); if (selected.coach && !coach.parentElement.querySelector('[data-remove="coach"]')) coach.insertAdjacentHTML('afterend', '<button type="button" data-remove="coach" class="mt-2 w-full text-xs font-bold text-white/55 hover:text-red-300">Remove coach</button>'); };
  const refresh = () => { render(); decorate(); };
  const syncDraft = async () => { const payload = {formation:formation.value, players:slotList().filter(slot => selected[slot]).map(slot => ({slot,player_id:selected[slot].id})), coach_player_id:selected.coach?.id || null}; try { const response = await fetch(@json(route('squads.draft.sync', $league)), {method:'PUT',headers:{'Content-Type':'application/json','Accept':'application/json','X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]')?.content},body:JSON.stringify(payload)}); const data = await response.json(); if (!response.ok) { (data.conflict_player_ids || []).forEach(id => conflicts.add(Number(id))); window.showToast?.(data.message || 'Unable to reserve player.', 'error'); } else conflicts.clear(); } catch (_) { window.showToast?.('Unable to save your draft.', 'error'); } refresh(); };
  document.addEventListener('click', async e => { const remove = e.target.closest('[data-remove]'); if (remove) { const slot = remove.dataset.remove; conflicts.delete(Number(selected[slot]?.id)); delete selected[slot]; await syncDraft(); return; } const button = e.target.closest('.slot-button'); if (button) open(button.dataset.slot); const result = e.target.closest('[data-player]'); if (result) { const slot = activeSlot === 'coach' ? 'coach' : activeSlot; conflicts.delete(Number(selected[slot]?.id)); selected[slot] = JSON.parse(result.dataset.player); close(); refresh(); await syncDraft(); } });
  search.addEventListener('input', () => { clearTimeout(timer); timer=setTimeout(() => runSearch(), 300); }); more.addEventListener('click', () => { page++; runSearch(true); }); document.getElementById('closePlayerModal').addEventListener('click', close); modal.addEventListener('click', e => { if (e.target === modal) close(); }); formation.addEventListener('change', async () => { Object.keys(selected).filter(k => k !== 'coach' && !slotList().includes(k)).forEach(k => delete selected[k]); refresh(); await syncDraft(); });
  document.getElementById('saveSquad').addEventListener('click', async () => { const button = document.getElementById('saveSquad'); button.disabled=true; button.textContent='Saving…'; const payload={formation:formation.value, players:slotList().map(slot=>({slot,player_id:selected[slot].id})), coach_player_id:selected.coach.id}; const response=await fetch(@json(route('squads.store',$league)),{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json','X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]')?.content},body:JSON.stringify(payload)}); const data=await response.json(); if(response.ok) { if (window.DashboardSPA) await window.DashboardSPA.navigate(data.redirect_url); else location.href=data.redirect_url; } else { (data.conflict_player_ids || []).forEach(id => conflicts.add(Number(id))); refresh(); window.showToast?.(data.message || Object.values(data.errors || {}).flat()[0] || 'Unable to save squad.', 'error'); button.disabled=false; button.textContent='Save and lock squad'; } });
  refresh();
})();
</script>
@endif
